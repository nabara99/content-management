    ...
    <div class="flex min-h-screen">
        ...
        <aside class="fixed inset-y-0 left-0 z-30 w-64 bg-white border-r border-gray-200 flex flex-col transition-transform duration-200"
            ...
            <nav class="flex-1 px-4 py-6 space-y-1 overflow-y-auto">
                ...
                {{-- Settings --}}
                <a href="{{ route('admin.settings') }}" class="flex items-center gap-3 px-3 py-2.5 rounded-xl text-sm font-medium transition-all duration-150
                    {{ request()->routeIs('admin.settings') ? 'text-white shadow-md' : 'text-gray-600 hover:bg-gray-100' }}"
                    @if(request()->routeIs('admin.settings')) style="background: linear-gradient(135deg, #2563EB, #1D4ED8);" @endif>
